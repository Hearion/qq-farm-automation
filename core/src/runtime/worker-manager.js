...
function createWorkerManager(options) {
    ...
    function handleWorkerMessage(accountId, msg) {
        ...
        if (msg.type === 'status_sync') {
            ...
            // 尝试更新昵称到 store
            if (msg.data && msg.data.status && (msg.data.status.name || msg.data.status.avatarUrl || msg.data.status.avatar_url)) {
                const newNick = String(msg.data.status.name).trim();
                const newAvatar = String(msg.data.status.avatarUrl || msg.data.status.avatar_url || '').trim();
                // 忽略无效昵称
                if ((newNick && newNick !== '未知' && newNick !== '未登录') || newAvatar) {
                    // 避免频繁写入，只在内存中无昵称或不一致时更新
                    if ((newNick && worker.nick !== newNick) || (newAvatar && worker.avatar !== newAvatar)) {
                        const oldNick = worker.nick;
                        if (newNick) worker.nick = newNick;
