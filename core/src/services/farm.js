...
const {
    isAutomationOn, getPreferredSeed, getAutomation, getPlantingStrategy,
    getBagSeedPriority, getBagSeedFallbackStrategy, getFertilizerBuyOrganicCount,
    getFertilizerBuyOrganicThresholdHours, getFertilizerBuyNormalCount,
    getFertilizerBuyNormalThresholdHours, getFertilizerBuyCheckIntervalMinutes,
    getFarmQuietHours,
} = require('../models/store');
const { sendMsgAsync, getUserState, networkEvents, getWsErrorState } = require('../utils/network');
const { types } = require('../utils/proto');
const { toLong, toNum, getServerTimeSec, toTimeSec, log, logWarn, sleep, randomDelay, randomInRange, randomChance } = require('../utils/utils');
...
async function fertilize(landIds, fertilizerId = NORMAL_FERTILIZER_ID) {
    ...
    for (const landId of landIds) {
        ...
        if (landIds.length > 1) await randomDelay(80, 350);
    }
    return successCount;
}
...
async function fertilizeOrganicLoop(landIds) {
    ...
    let successCount = 0;
    let idx = 0;
    const maxIterations = ids.length * 3;
...
function inFarmQuietHours(now = new Date()) {
    const cfg = getFarmQuietHours();
...
function scheduleNextFarmCheck(delayMs) {
    ...
    const actualDelay = delayMs != null ? delayMs : randomInRange(CONFIG.farmCheckIntervalMin, CONFIG.farmCheckIntervalMax);
    farmScheduler.setTimeoutTask('farm_check_loop', Math.max(0, actualDelay), async () => {
        if (!farmLoopRunning) return;
        await checkFarm();
        if (!farmLoopRunning) return;
...
function onLandsChangedPush(lands) {
    ...
    farmScheduler.setTimeoutTask('farm_push_check', randomInRange(2000, 8000), async () => {
