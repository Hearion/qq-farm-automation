...
const { toLong, toNum, toTimeSec, getServerTimeSec, log, logWarn, sleep, randomDelay, randomInRange, randomChance } = require('../utils/utils');
...
async function checkFriends(options = {}) {
    ...
    try {
        ...
        if (stealFriends.length > 0 && effectiveStealEnabled) {
            ...
            for (let si = 0; si < stealFriends.length; si++) {
                if (!canOperate(10008)) break;

                try {
                    await visitFriendForSteal(stealFriends[si], totalActions, state.gid, state.accountId);
                } catch {
                    // 单个好友失败不影响整体
                }
                if ((si + 1) % randomInRange(5, 8) === 0) {
...
async function friendCheckLoop() {
    ...
    await checkFriends();
    if (!friendLoopRunning) return;
    friendScheduler.setTimeoutTask('friend_check_loop', Math.max(0, randomInRange(CONFIG.friendCheckIntervalMin, CONFIG.friendCheckIntervalMax)), () => friendCheckLoop());
}
