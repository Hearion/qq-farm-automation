...
const IPHONE_MODELS = ['iPhone14,2', 'iPhone15,2', 'iPhone16,1', 'iPhone16,2', 'iPhone17,1', 'iPhone17,2'];
const IOS_VERSIONS = ['17.5.1', '18.0.1', '18.2.1', '18.4.1', '26.2.1', '26.3.0'];
const MEMORY_OPTIONS = ['4072', '6144', '7672', '8192'];

function generateRandomDeviceInfo() {
    return {
        sys_software: `iOS ${IOS_VERSIONS[Math.floor(Math.random() * IOS_VERSIONS.length)]}`,
        network: Math.random() > 0.3 ? 'wifi' : '5G',
        memory: MEMORY_OPTIONS[Math.floor(Math.random() * MEMORY_OPTIONS.length)],
        device_id: `iPhone X<${IPHONE_MODELS[Math.floor(Math.random() * IPHONE_MODELS.length)]}>`,
    };
}
...
let accountFallbackConfig = {
    ...
    farmQuietHours: { ...DEFAULT_ACCOUNT_CONFIG.farmQuietHours },
    knownFriendGids: [],
    knownFriendGidSyncCooldownSec: DEFAULT_KNOWN_FRIEND_GID_SYNC_COOLDOWN_SEC,
    friendsListCacheTtlSec: DEFAULT_FRIENDS_LIST_CACHE_TTL_SEC,
};
...
function getFarmQuietHours(accountId) {
    const config = getAccountConfigSnapshot(accountId);
    if (!config.farmQuietHours) {
        return { enabled: true, start: '01:00', end: '07:30' };
    }
    return { ...config.farmQuietHours };
}

function getDeviceInfo(accountId) {
    const config = getAccountConfigSnapshot(accountId);
    if (config.deviceInfo && typeof config.deviceInfo === 'object' && config.deviceInfo.device_id) {
        return config.deviceInfo;
    }
    const generated = generateRandomDeviceInfo();
    applyConfigSnapshot({ deviceInfo: generated }, { persist: true, accountId });
    return generated;
}
...
function inferQqFromAccount(account = {}) {
    const direct = String(account.uin || account.qq || '').trim();
    if (/^\d{5,12}$/.test(direct)) return direct;

    if (String(account.platform || 'qq') !== 'qq') return '';
...
function normalizeAccountsData(raw) {
    ...
    const accounts = Array.isArray(data.accounts)
        ? data.accounts.map(a => {
            const account = a && typeof a === 'object' ? { ...a } : {};
            const qq = inferQqFromAccount(account);
            if (qq) {
                account.uin = account.uin ? String(account.uin) : qq;
                account.qq = account.qq ? String(account.qq) : qq;
            }
            return account;
        })
        : [];
...
function addOrUpdateAccount(acc) {
    ...
    const inferredQq = inferQqFromAccount(acc);
    if (acc.id) {
        ...
        if (idx >= 0) {
            ...
            const qq = inferQqFromAccount(data.accounts[idx]);
            if (qq) {
                data.accounts[idx].uin = data.accounts[idx].uin ? String(data.accounts[idx].uin) : qq;
                data.accounts[idx].qq = data.accounts[idx].qq ? String(data.accounts[idx].qq) : qq;
            }
        ...
        data.accounts.push({
            ...
            uin: acc.uin ? String(acc.uin) : inferredQq,
            qq: acc.qq ? String(acc.qq) : (acc.uin ? String(acc.uin) : inferredQq),
