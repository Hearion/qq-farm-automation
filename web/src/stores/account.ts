...
export function getAccountQq(account?: Account | null) {
  const direct = String(account?.uin || account?.qq || '').trim()
  if (/^\d{5,12}$/.test(direct))
    return direct

  if ((account?.platform || 'qq') !== 'qq')
    return ''
...
export function getAccountAvatarUrl(account?: Account | null) {
  const direct = String(account?.avatar || account?.avatarUrl || account?.avatar_url || '').trim()
  if (direct)
    return direct

  const uin = getAccountQq(account)
  if (uin)
